...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    document.getElementById('editorGroup').style.display = 'block';
    ...
    try {
        ...
        if (data.content) {
            ...
            if (document.getElementById('allowAdditionalAttributes')) {
                document.getElementById('allowAdditionalAttributes').checked = !!(data.meta && data.meta.allowAdditionalAttributes);
            }
            if (document.getElementById('enableGeofencing')) {
                ...
                // to prevent browser load. The script handles threading internally.
                if (groupByInput) {
...
async function loadScriptAsCopy(filename, isDraft) {
    ...
    function addStep(type) {
        ...
        <div style="display: flex; justify-content: space-between; margin-bottom: 5px; align-items: center;">
            <strong style="color: ${type === 'API' ? '#38bdf8' : '#eab308'}">${type} Step</strong>
        ...
        try {
            const res = await fetch('/api/scripts/generate', {
                ...
                body: JSON.stringify({
                    ...
                    existing_code: document.getElementById('pythonCode').value,
    ...
    async function runTest() {
        ...
        try {
            ...
            // Review Section is now always enabled
        }
    }
    ...
    function renderTimeline(logs) {
        ...
        pre.innerText = logs; // Dump raw text for now as user requested raw logs
    ...
    async function importScript() {
        ...
        // Capture Columns (Priority: Manual > Detected > Global)
        const manualVal = document.getElementById('manualColumns').value;
        ...
        let finalColsStr = manualVal || detectedVal || globalVal || "";
        let inputColumns = [];
    ...
    window.openOutputConfigModal = function () {
        ...
        if (manualVal) manualVal.split(',').forEach(s => allCols.add(s.trim()));
    ...
    function renderVisualsFromSteps(steps, inputColumns) {
        const container = document.getElementById('analysisSteps');
        container.innerHTML = ''; // Clear
    ...
    async function autoPopulateStepsFromAI(code) {
        ...
        try {
            ...
            if (result.steps && result.steps.length > 0) {
                ...
                renderVisualsFromSteps(result.steps, result.excelColumns);
